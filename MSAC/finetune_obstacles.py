...
class SaveVecNormalizeCallback(BaseCallback):
    ...
    def _on_step(self) -> bool:
        if self.n_calls % self.save_freq == 0:
            ...
            if self.verbose > 0: print(f"Saved VecNormalize to {path}")
        return True
...
def perform_vec_surgery(old_vec_path, new_env, num_old_obs=18):
    """Pads VecNormalize so the new Lidar inputs don't crash the scaling logic"""
    ...
    # Initialize Lidar mean/var to safe defaults
...
def finetune():
    ...
    model = SAC(
        ...
        replay_buffer_kwargs=dict(n_steps=3, gamma=0.99),
        gamma=0.99,
        verbose=1,
        device="cuda",
        learning_rate=1e-4, 
        tensorboard_log="./multistep_sac_obstacles_tensorboard/",
    ...
    print(f"Finetuning on {num_cpu} cores with MSAC (n=5)...")
    model.learn(
